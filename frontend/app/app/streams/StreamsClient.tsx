...
type StreamsTab = 'active' | 'paused' | 'history';
...
export default function StreamsPage() {
  ...
  const [tab, setTab] = useState<StreamsTab>('active');
  ...
  const historyStreams = streams.filter(s => s.status === 'Stopped' || isDepleted(s, nowSec));
            ...
            <div className="space-y-4">
              {activeStreams.length === 0 ? (
                <div className="text-center py-16 text-provn-muted">
                  ...
                  <p className="mb-1">No active streams</p>
          ...
          {tab === 'paused' && (
            <div className="space-y-4">
              {pausedStreams.length === 0 ? (
                <div className="text-center py-16 text-provn-muted">
                  <Waves className="w-12 h-12 mx-auto mb-3 opacity-30" />
                  <p className="mb-1">No paused streams</p>
                </div>
              ) : pausedStreams.map((s) => (
                <StreamCard
                  key={s.id}
